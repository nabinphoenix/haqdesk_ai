...
type Thread = { id:number; title:string|null; is_group:boolean; participants:Person[]; last_message:string|null; unread_count:number };
...
export default function MessagesPage() {
  ...
  useEffect(()=>{ const initTimer=window.setTimeout(()=>{ setRole(localStorage.getItem("userRole")); void loadThreads(); void fetchWithAuth("/api/v1/internal-messages/recipients").then(r=>r.ok?r.json():[]).then(setRecipients); void fetchWithAuth("/api/v1/auth/me").then(r=>r.ok?r.json():null).then(data=>{ if(data?.id) setCurrentUserId(data.id); }); },0); return()=>window.clearTimeout(initTimer); },[loadThreads]);
  ...
  async function start(id:string){if(!id)return;const r=id==="__all_employees__"?await fetchWithAuth("/api/v1/internal-messages/threads/broadcast",{method:"POST"}):await fetchWithAuth("/api/v1/internal-messages/threads",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({recipient_id:Number(id)})});if(r.ok){const t=await r.json();await loadThreads();open(t.id);}}
  ...
  return <div className="mx-auto flex h-full w-full max-w-6xl gap-4 p-4 text-foreground">
    <aside className="w-80 rounded-2xl border border-border bg-surface p-3"><div className="mb-3 flex items-center justify-between"><h1 className="text-lg font-bold">Messages</h1><span className={`text-xs ${connected?"text-[var(--success-foreground)]":"text-[var(--warning)]"}`}>{connected?"Live":"Reconnecting…"}</span></div>
      <select aria-label="Start a conversation" defaultValue="" onChange={e=>{start(e.target.value);e.target.value=""}} className="mb-3 w-full rounded-lg border border-border bg-background p-2 text-sm"><option value="">New conversation…</option>{role==="business_admin"&&<option value="__all_employees__">All employees</option>}{recipients.map(p=><option key={p.id} value={p.id}>{p.name} · {p.role.replace("_"," ")}</option>)}</select>
      <div className="space-y-1">{threads.map(t=>{const p=t.participants[0];const label=t.title||p?.name||"Conversation";return <button key={t.id} onClick={()=>open(t.id)} className={`w-full rounded-xl p-3 text-left ${active===t.id?"bg-accent text-on-accent":"hover:bg-surface-wash"}`}><div className="flex justify-between"><b>{label}</b>{t.unread_count>0&&<span className="rounded-full bg-accent px-2 text-xs">{t.unread_count}</span>}</div><p className="truncate text-xs opacity-70">{t.last_message||p?.role}</p></button>})}</div>
    </aside>
    <section className="flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-surface">{active?<><header className="flex items-center gap-3 border-b border-border p-4"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent text-sm font-bold text-on-accent">{current?.title?.slice(0,1)||current?.participants[0]?.name?.slice(0,1).toUpperCase()||<User size={16}/>}</div><div><div className="font-semibold">{current?.title||current?.participants[0]?.name||"Conversation"}</div><div className="text-xs capitalize text-muted-foreground">{current?.is_group?"Group conversation":current?.participants[0]?.role.replace("_"," ")}</div></div></header>
      <div className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">{messages.map(m=>{const other=current?.participants[0];const mine=currentUserId!==null?m.sender.id===currentUserId:m.sender.id!==other?.id;const initials=m.sender.name.split(" ").map(x=>x[0]).join("").slice(0,2).toUpperCase();return <div key={m.id} className={`flex w-full items-end gap-2.5 ${mine?"justify-end":"justify-start"}`}>
